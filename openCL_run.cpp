//Project 7B - OpenCL trials - By Armand Parajon
...
main( int argc, char *argv[ ] )
{
    ...
    size_t sumsSize = size * sizeof(cl_float);

	// fill the host memory buffers:

	for (int i = 0; i < size; i++) {
    	fscanf(sFile, "%f", &hArray[i]);
    ...
    FILE* output = fopen("openCL_results.csv", "w");
    fprintf(output, "Peak Performance (Mega AutoCors Per Second)\n");
    fprintf(output, "%lf\n", (double)size * size / (time1 - time0) / 1000000);

    fprintf(output, "Shift,Sum\n");
    for(int i = 0; i < size; i++)
        fprintf(output, "%d,%f\n", i, hSums[i]);
